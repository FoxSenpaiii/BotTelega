...
async def cmd_start(message: types.Message):
    ...
    await message.reply("Приветвую тебя, заблудшая душа. Назови свое имя")
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await Form.next()
    await message.reply("Как твоя фамилия?")
...
async def process_age(message: types.Message, state: FSMContext):
    ...
    await message.reply("В каком годы ты поступил к нам?", reply_markup=markup)



@dp.message_handler(lambda message: message.text not in ["2017", "2018", "2019", "2020"], state=Form.startedu)
async def process_gender_invalid(message: types.Message):
    return await message.reply("Ответ не верный. Выберите ныжный вариант на клавиатуре. ")
    ...
    async with state.proxy() as data:
        ...
        await bot.send_message(
            ...
            md.text(
                ...
                md.text('Начала обучения в :' , data['startedu']),
                sep='\n',
            ),
            reply_markup=markup,
